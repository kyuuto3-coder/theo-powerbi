...
5. 고치고 싶은 점을 채팅으로 말하면 Claude가 다시 만들어 줍니다. 마지막에 "수동 제작 가이드"를 원하면 `output/<이름>/manual-guide.html`(브라우저에서 열리는 단계별 체크리스트)도 받을 수 있습니다.

## 여러 파일이 있을 때
- 파일이 여러 개면 Claude가 **열 이름이 달라도 값이 겹치는 키**(예: `cust_no` ↔ `고객ID`)와 **복합 키**(예: 월 + 지역코드)를 찾아 관계를 맺고, **같은 구조의 파일들**(예: `orders_2025-01.csv`, `orders_2025-02.csv`)은 하나의 표로 합칩니다. 이후 같은 이름 규칙의 파일을 `rawdata/`에 넣으면 Power BI에서 **Refresh**만 눌러도 반영됩니다.
- 관계가 애매하면 Claude가 한 번 물어봅니다. 확실히 알고 있으면 처음부터 적어 주세요: *"sales.csv가 중심이고 products.xlsx와는 상품코드로 연결"*.
