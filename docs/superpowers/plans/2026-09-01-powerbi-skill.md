...
### Task 16: Beginner guide for coworkers who have never used an AI tool (Korean, Sonnet 5 premise)

**Files:**
- Create: `docs/시작하기.md`
- Modify: `README.md` (add a link at the top), `.claude/skills/powerbi/SKILL.md` (Sonnet note)

- [ ] **Step 1: Write `docs/시작하기.md`**
...
Insert right after the first paragraph of `README.md`:

```markdown
> 🆕 AI 도구가 처음이라면 **[docs/시작하기.md](docs/시작하기.md)** 를 먼저 보세요. 설치부터 첫 리포트까지 그림 없이도 따라 할 수 있게 단계별로 적어 두었습니다.
```

- [ ] **Step 3: Add the Sonnet premise to `SKILL.md`**

In `SKILL.md`, under the `# Power BI report builder` heading, append this sentence to the intro paragraph:

```markdown
This skill is written to be followed literally by Claude Sonnet: when a step says run a command, run exactly that command; when it says decide, decide with the defaults given here rather than asking.
```

- [ ] **Step 4: Verify and commit**

```bash
grep -c '' docs/시작하기.md && grep -n '시작하기' README.md && grep -n 'Sonnet' .claude/skills/powerbi/SKILL.md
git add docs/시작하기.md README.md .claude/skills/powerbi/SKILL.md
git commit -m "docs: beginner step-by-step guide (Korean, Sonnet 5 premise)"
```
Then re-run Task 15 Step 1 to push.

## Self-review notes

- **Spec coverage:** §2 layout → Tasks 1, 13; beginner guide (user request 2026-09-01) → Task 16; §3 README → Task 13; §4 workflow + token rules → Task 13 (SKILL.md); §5 spec contract → Tasks 8, 13; §6 profiler → Tasks 4–7; §7 modes → Task 8 (Get-SpecModel) + SKILL.md guidance; §8 builder → Tasks 9–11; §9 testing → every task + Task 14; §10 guide → Task 12; §12 distribution → Tasks 1, 15.
