...
1. **PROFILE** — run:
   ...
   Output = files → tables (columns, type, nulls, distinct, samples, `KEY` / `KEY (composite): A + B`), dictionary sheets, csv `encoding`, `header_row`, dirt notes (`DIRTY:` off-type values, `placeholders:`, `whitespace:`, `case-variants:`, table `DIRT:` blank/duplicate rows), `GROUPS` (files with identical columns → one table via `filePattern`), and `KEY MATCHES` (`many.col -> one.key (NN% of sampled values found[; names differ][; composite key][; case-insensitive - clean both columns])`). Exit code 2 = no files → ask the user (Korean) to put files in `rawdata/`.
...
3. **DESIGN** — decide:
   ...
   - *Cleaning:* real data is rarely clean — map every dirt note in the profile to a clean option (see spec-schema.md). Table `DIRT:` blank/duplicate rows → `"clean": {"removeBlankRows": true, "dropDuplicates": true}`. Column `DIRTY: n non-date` → look at the examples: date-shaped strings → `dateFormats` with those formats; words like "TBD" → `nullValues`. `DIRTY: n non-<number>` with currency/percent strings → `numberClean: true`. `placeholders:` → `nullValues` (unless the token is a real category the lookup table also has, e.g. "Undefined" market segment) — or `valueMap` when codes should become readable labels. `whitespace:`/`case-variants:` on a relationship or slicer column → `trim: true` (+ `case: "upper"` for codes), applied to BOTH sides of the relationship. On any fact table ≥ 1000 rows also set `"errorsToNull": true` so one bad value never blocks Refresh. A KEY MATCH tagged `case-insensitive` counts as a ≥ 90% match once both columns get `trim`+`case`. Impossible numeric values (negative quantity, min/max far outside plausibility) → `validRange` and say so in the handoff.
   - *Duplicates — resolve logically:* `KEY after dedupe` note → the id becomes unique with `dropDuplicates` alone; keep `key: true`. `CONFLICTING dup ids` note → same id with different data: NEVER rely on blind `dropDuplicates` — use `"dedupeBy": {"keys": [...], "keep": "last", "orderBy": <update/status date column>}` (or `keep: "mostComplete"` when no date exists), load the orderBy column, and state the rule in the handoff. If neither rule fits, ask the user which row wins.
   - *Granularity:* when the user asks for a summary at a coarser grain (monthly totals, per-store rollup), add a summary table with `from`/`groupBy`/`aggregations` instead of loading the file twice — it inherits the source's cleaning. Derived per-row values (e.g. `Total Guests = [Adults]+[Children]`) → `derived` on the source table.
